...
import { guardarFichaPerfil, actualizarEstadoSocio, anularSocio } from '@/lib/actions/socios';
...
export default async function SocioDetallePage({
  ...
  if (!socio) notFound();
  const anulado = !!socio.anulado_en;
  ...
  return (
    <div className="space-y-8 max-w-4xl">
      <div>
        ...
        {socio.updated_at && (
          <p className="text-xs text-neutral-400 mt-1">
            Última edición: {new Date(socio.updated_at).toLocaleString('es-CL')} · {socio.editor?.nombre_completo ?? '—'}
          </p>
        )}
      </div>
      ...
      <section>
        ...
        {anulado ? (
          <p className="text-sm text-neutral-500">Registro anulado: no corresponde editar la ficha.</p>
        ) : (
      ...
      {!anulado && <AnularForm action={anularSocio} idField="id" idValue={socio.id} label="socio" />}
    </div>
  );
}
